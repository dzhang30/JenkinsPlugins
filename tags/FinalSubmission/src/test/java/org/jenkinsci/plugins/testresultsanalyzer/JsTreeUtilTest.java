...
public class JsTreeUtilTest {
	...
	public String getteststring(String name, JSONObject value)
	{
		...
		JSONObject tree = temp_tree.getJsTree(build,check,testStatus_p,resultInfo,build).getJSONArray("results").getJSONObject(0);
	...
	public JSONArray getchildlist(boolean check, String testStatus)
	{
		...
		JsTreeUtil temp_tree = new JsTreeUtil();
		JSONObject tree = temp_tree.getJsTree(build,check,testStatus,resultInfo,build).getJSONArray("results").getJSONObject(0);
