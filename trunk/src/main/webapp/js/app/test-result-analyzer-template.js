var tableContent = '<div class="table-row {{parentclass}}-{{addName text}}" parentclass= "{{parentclass}}" parentname="{{parentname}}" name = "{{addName text}}" {{#if isChild}} style="display:none"{{/if}}>' +
					...
					'{{/if}}' +
					'><a {{getURL parentname text hierarchyLevel}}">&nbsp;{{text}}</a> {{description}}</div>' + //Add other words to the titles here
					'' +
					'{{#each this.buildResults}}' +
					'\n' + '         <div class="table-cell build-result {{applystatus status}}" data-result=\'{{JSON2string this}}\'>{{applyvalue status totalTimeTaken this}}</div>' +
					'{{/each}}' +
					'\n' + '</div>' +
					'{{#each children}}\n' +
...
Handlebars.registerHelper('getURL', function (parent, name, hierarchyLevel) {
	...
	if (hierarchyLevel == 1)
		toReturn = 'href="' + getUrl(window.location.href, parent, name) + '"';

	return new Handlebars.SafeString(toReturn);
});

Handlebars.registerHelper('applyvalue', function (status, totalTimeTaken, object) {
	var json = JSON.stringify(object);
	var arr = JSON.parse(json);
	var ret = status;
	if(typeof arr.failureDetails != 'undefined')
	{
		status += '\n' + arr.failureDetails;
	}
	return status;
